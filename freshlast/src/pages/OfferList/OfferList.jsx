...
import { getAllMerchants, getProfile } from "../../api/profile";
...
export default function OfferList({
  ...
  const [selectedListingCategory, setSelectedListingCategory] =
    useState("All Produce");
  const [selectedMerchantCategory, setSelectedMerchantCategory] =
    useState("All Produce");
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setMerchants((await getAllMerchants()) || []);
      } catch (e) {
        console.error(e);
      }
      setIsLoading(false);
  ...
  const filteredListings = useMemo(
    () =>
      listings.filter((l) => {
        ...
        const sel = selectedListingCategory.toLowerCase();
        const matchCat =
          sel === "all produce" || cat.includes(sel.replace(/s$/, ""));
        const matchSearch = (l.name || "")
          .toLowerCase()
          .includes(searchQuery.toLowerCase());
        return matchCat && matchSearch;
      }),
    [listings, selectedListingCategory, searchQuery],
  );

  const filteredMerchants = useMemo(
    () =>
      merchants.filter((m) => {
        const sel = selectedMerchantCategory.toLowerCase();
        const matchCat =
          sel === "all produce" ||
          (Array.isArray(m.category)
            ? m.category.some((c) =>
                c.toLowerCase().includes(sel.replace(/s$/, "")),
              )
            : (m.category || "").toLowerCase().includes(sel.replace(/s$/, "")));
        const matchSearch = (m.name || "")
          .toLowerCase()
          .includes(searchQuery.toLowerCase());
        return matchCat && matchSearch;
      }),
    [merchants, selectedMerchantCategory, searchQuery],
  );
  ...
  const visible = showAll
    ? sortedListings
    : sortedListings.slice(0, PREVIEW_COUNT);
        ...
        <div className="offerlist__container">
          ...
          <div className="offerlist__categories-inner">
            {CATEGORIES.map((c) => (
              <button
                key={c}
                className={`offerlist__filter-btn ${selectedListingCategory === c ? "offerlist__filter-btn--active" : ""}`}
                onClick={() => {
                  setSelectedListingCategory(c);
                  setShowAll(false);
                }}
              >
                {c}
        ...
        <div className="offerlist__container">
          ...
          <div className="offerlist__categories-inner">
            {CATEGORIES.map((c) => (
              <button
                key={c}
                className={`offerlist__filter-btn ${selectedMerchantCategory === c ? "offerlist__filter-btn--active" : ""}`}
                onClick={() => {
                  setSelectedMerchantCategory(c);
                  setShowAll(false);
                }}
              >
                {c}
              ...
              <div className="offerlist__grid">
                {filteredMerchants.map((m) => (
                  <MerchantItem
                    key={m.id}
                    merchant={m}
                    onSelect={(x) => navigate(`/viewMerchant/${x.id}`)}
                  />
                ))}
              </div>
            </>
